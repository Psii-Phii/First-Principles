...
def gen_book_tex(b, cfg, moodcfg) -> str:
    ...
    return "\n".join([
        ...
        "\\booktitlepage{%s}{%s}{%s}{%s}{%s}{%s}{%s}{%s}" % (
            ...
            tex_escape(str(b.get("year", "") or "")), tex_escape(str(b.get("read", "") or "")),
...
ORNAMENT = """<svg class="ornament" width="72" height="14" viewBox="0 0 72 14" aria-hidden="true">
...
def page(cfg, *, title, body, depth=0, description="") -> str:
    """depth = how many folders below the commonplace root (0 = commonplace/, 1 = commonplace/books/)."""
    prefix = "../" * depth                 # to the commonplace folder
...
def write_site(cfg, books, moodcfg):
    sdir = (ROOT / cfg.get("site_dir", "../commonplace")).resolve()
    bdir = ROOT / cfg.get("build_dir", "build")
    ...
    css = (read_asset("commonplace.css")
           .replace("/*{{font-faces}}*/", font_faces(moodcfg))
           .replace("/*{{mood-rules}}*/", mood_css(moodcfg)))
    (sdir / "commonplace.css").write_text(css, encoding="utf-8")
    ...
    # ── library (commonplace/index.html) ──
    items = ""
    for b in by_recent:
        n = len(b["entries"])
        when = html.escape(f"read {b['read']}") if b.get("read") else (fmt_date(latest_date(b)) if latest_date(b) else "")
        items += f"""
<li class="mood-{b['mood']}">
  ...
  <p>{book_line(b)} <span class="sep">&middot;</span> {n} {'entry' if n == 1 else 'entries'}</p>
    ...
    body = f"""
<p class="eyebrow">{html.escape(cfg.get('subtitle', 'The commonplace book'))}</p>
...
<ul class="booklist">{items}</ul>"""
    ...
    # ── all entries (commonplace/entries.html) ──
...
<p class="empty" hidden>Nothing here with that mood yet.</p>"""
    ...
    for b in books:
        ...
        read = f'<span class="sep">&middot;</span>read {html.escape(str(b["read"]))}' if b.get("read") else ""
        n = len(entries)
        body = f"""
