...
A commonplace book: one document per book you read, holding the quotes and
excerpts you kept from it and your commentary on them. Each book is a plain-text
YAML file, published two ways — a typeset PDF per book and the `/commonplace/`
section of the First Principles site (this folder's parent).

```
commonplace-src/             ← this folder: sources, never served
...
python3 build.py              # regenerate ../commonplace/ and build/<book>.tex
...
in date order) and `../commonplace/books/<slug>.html`. `/commonplace/` is the
library of books, `/commonplace/entries.html` every entry in one stream with mood
filters; a built PDF is copied to `../commonplace/pdf/` and linked from its book page.

## Publishing

The generated `../commonplace/` folder is committed like the rest of the site, so
publishing is a push. The app does this for you; by hand it's
`python3 build.py --pdf && git add -A .. && git commit -m "Commonplace: …" && git push`.

Preview the whole site with `make serve` (runs `python3 -m http.server` in the site
root) and open <http://localhost:8000/commonplace/>.
