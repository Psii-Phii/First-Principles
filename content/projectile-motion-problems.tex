...
\begin{tikzpicture}[scale=1.02,>=Stealth]
  \coordinate (d) at (-1.35,-0.90);
  % ground plane
  \fill[gray!13] (-0.5,0) -- (5.6,0) -- ($(5.6,0)+(d)$) -- ($(-0.5,0)+(d)$) -- cycle;
  \draw[gray!50] (5.6,0) -- ($(5.6,0)+(d)$) ($(5.6,0)+(d)$) -- ($(-0.5,0)+(d)$);
  % axes (y vertical, z out of the page, to match the solution below)
  \draw[->,thick] (0,0) -- (6.0,0) node[right] {$x$};
  \draw[->,thick] (0,0) -- ($1.15*(d)$) node[below left] {$z$};
  \draw[->,thick] (0,0) -- (0,3.3) node[above] {$y$};
  % trajectory: x = 4.6 t, y = 8.8 t(1-t), t in [0,1]
  \draw[very thick,blue!70!black,domain=0:1,smooth,variable=\t,samples=60]
      plot ({4.6*\t},{8.8*\t*(1-\t)});
  % equal-time dots on trajectory + projections
  \foreach \t in {0.125,0.25,0.375,0.5,0.625,0.75,0.875}{
    \fill[blue!70!black] ({4.6*\t},{8.8*\t*(1-\t)}) circle (1.6pt);
    \draw[gray!60,densely dotted] ({4.6*\t},{8.8*\t*(1-\t)}) -- ({4.6*\t},0);
    \draw[gray!60,densely dotted] ({4.6*\t},{8.8*\t*(1-\t)}) -- (-0.22,{8.8*\t*(1-\t)});
    \fill[orange!85!black] ({4.6*\t},0) circle (1.6pt);
    \fill[red!75!black] (-0.22,{8.8*\t*(1-\t)}) circle (1.6pt);
  }
  \fill[orange!85!black] (0,0) circle (1.6pt);
  \fill[orange!85!black] (4.6,0) circle (1.6pt);
  % launch vector
  \draw[very thick,->,blue!70!black] (0,0) -- (50:1.25);
  \node[blue!70!black] at (50:1.55) {$\vec u$};
  % captions for the two shadows
  \node[orange!85!black,align=center] at (4.9,-0.62)
      {\footnotesize equal steps:\\[-3pt]\footnotesize uniform in $x$};
  \node[red!75!black,align=center,anchor=east] at (-0.42,2.55)
      {\footnotesize bunches at the top:\\[-3pt]\footnotesize decelerating in $y$};
  % eyes
  \eye{(2.3,3.55)}{-90}{0.55}
  \node[anchor=west] at (3.0,3.62) {\small view from the top: only $x$ changes};
  \eye{(6.9,1.35)}{180}{0.55}
  \node[anchor=west,align=left] at (6.15,0.55)
      {\small view from the front:\\[-2pt]\small only $y$ changes};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.16,>=Stealth]
  \draw[very thick,->,red!75!black] (0,15) -- (0,0) node[midway,left] {\small $\vec v_{r/g}$};
  \draw[very thick,->,orange!85!black] (0,0) -- (15,0) node[midway,below] {\small $\vec v_g$};
  \draw[very thick,->,blue!70!black] (0,15) -- (15,0) node[midway,above right] {\small $\vec v_r = \vec v_g + \vec v_{r/g}$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.42,>=Stealth]
  \draw[thick] (-2.0,0) -- (13.5,0);
  \fill[pattern=north east lines] (-2.0,-0.6) rectangle (13.5,0);
  \fill[gray!15] (-2.0,0) rectangle (0,10);
  \draw[thick] (0,10) -- (0,0);
  \draw[thick] (-2.0,10) -- (0,10);
  \node[below left] at (0.3,-0.6) {\small $O$};
  \draw[->] (0,0) -- (2.6,0) node[below] {\small $x$};
  \draw[->] (0,0) -- (0,2.6); \node at (0.7,2.3) {\small $y$};
  \draw[<->] (-1.3,0.15) -- (-1.3,9.85) node[midway,left] {\small $10~\mathrm{m}$};
  \draw[very thick,->,blue!70!black] (0,10) -- (4.0,10) node[below right=-3pt] {\small $u$};
  \draw[very thick,->,red!75!black] (0,10) -- ({4.4*cos(60)},{10+4.4*sin(60)}) node[above=1pt] {\small $u$};
  \draw (1.8,10) arc (0:60:1.8); \node at (2.75,10.95) {\small $60^\circ$};
  \draw[blue!70!black,very thick,dashed,domain=0:1,smooth,variable=\s,samples=40]
      plot ({8.660*\s},{10-5*\s*\s});
  \draw[red!75!black,very thick,dashed,domain=0:2,smooth,variable=\t,samples=60]
      plot ({4.330*\t},{10+7.5*\t-5*\t*\t});
  \fill (8.660,5) circle (4.5pt) node[right=4pt] {\small $P$};
  \draw[densely dotted] (8.660,5) -- (8.660,0);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.9,>=Stealth]
  ...
  \draw[thick] (-0.6,-0.4) -- (8.6,-0.4);
  \foreach \x in {1.2,6.8} \draw[thick,fill=white] (\x,-0.2) circle (0.2);
  \draw[very thick,->] (8.3,1.6) -- (9.5,1.6) node[right] {$a$};
  % Newton
  \draw (1.45,0) -- (1.6,0.45) -- (1.75,0);
  \draw (1.6,0.45) -- (1.6,1.05);
  \draw (1.6,1.2) circle (0.15);
  \draw (1.6,0.85) -- (1.85,0.95);
  % throw
  \draw[very thick,->,blue!70!black] (1.85,0.95) -- ++(60:1.3) node[above right=-2pt] {\small $10~\mathrm{m/s}$};
  \draw[densely dotted] (1.85,0.95) -- (2.8,0.95);
  \draw (2.35,0.95) arc (0:60:0.5); \node at (2.65,1.3) {\small $60^\circ$};
  % walking distance
  \draw[densely dotted] (1.6,0) -- (1.6,-1.2);
  \draw[densely dotted] (2.635,0) -- (2.635,-1.2);
  \draw[<->] (1.6,-1.1) -- (2.635,-1.1) node[midway,below] {\small $1.15$ m};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.75,>=Stealth]
  \draw[->] (-0.3,0) -- (3.3,0) node[right] {\small $x$ (train)};
  \draw[->] (0,-0.2) -- (0,4.2) node[above] {\small $y$};
  \draw[very thick,blue!70!black,domain=0:1.732,smooth,variable=\t,samples=50]
      plot ({5*\t-2.5*\t*\t},{8.66*\t-5*\t*\t});
  \fill (1.15,0) circle (2pt) node[below] {\small $1.15$};
  \draw[densely dotted] (2.5,0) -- (2.5,3.66);
  \node[below] at (2.5,0) {\small $2.5$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.85,>=Stealth]
  \fill[red!7] (4,0) rectangle (10.0,3.7);
  \draw[red!50,densely dashed] (4,0) -- (4,3.7);
  \node[red!60!black,anchor=west] at (4.2,3.4) {\small region with $g' = g/0.81$};
  \node[anchor=east] at (3.8,3.4) {\small ordinary gravity $g$};
  \draw[thick] (-0.5,0) -- (10.0,0);
  \fill[pattern=north east lines] (-0.5,-0.22) rectangle (10.0,0);
  \draw[very thick,blue!70!black,domain=0:4,smooth,variable=\x,samples=40]
      plot ({\x},{\x*(8-\x)/8});
  \draw[thick,gray!60,dashed,domain=4:8,smooth,variable=\x,samples=40]
      plot ({\x},{\x*(8-\x)/8});
  \node[gray!70!black,anchor=west] at (7.9,1.3) {\small path if $g$};
  \node[gray!70!black,anchor=west] at (7.9,0.95) {\small continued};
  \draw[very thick,red!75!black,domain=0:1,smooth,variable=\t,samples=40]
      plot ({4+3.6*\t},{2-2*\t*\t});
  \fill (4,2) circle (2pt);
  \node[anchor=east] at (3.85,2.3) {\small top: $v_y = 0$};
  \draw[very thick,->,blue!70!black] (0,0) -- (0.95,0.95);
  \draw (0.6,0) arc (0:45:0.6); \node at (0.95,0.24) {\small $\theta$};
  \draw[<->] (0,-0.55) -- (4,-0.55) node[midway,below] {\small $d/2$};
  \draw[<->] (4,-0.55) -- (7.6,-0.55) node[midway,below] {\small $?$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.95,>=Stealth]
  % peaks
  \fill[gray!18] (-1.6,0) -- (0,6) -- (3.464,0) -- cycle;
  \draw[thick] (-1.6,0) -- (0,6) -- (3.464,0);
  \fill[gray!18] (6,0) -- (8,2) -- (9.6,0) -- cycle;
  \draw[thick] (6,0) -- (8,2) -- (9.6,0);
  % valley floor
  \draw[thick] (-2.6,0) -- (10.6,0);
  \fill[pattern=north east lines] (-2.6,-0.25) rectangle (10.6,0);
  \node[above left] at (0,6) {$A$};
  \node[above right] at (8,2) {$B$};
  % face angles
  \draw (2.664,0) arc (180:120:0.8); \node at ($(3.464,0)+(150:1.2)$) {\small $60^\circ$};
  \draw (6.8,0) arc (0:45:0.8); \node at ($(6,0)+(22:1.25)$) {\small $45^\circ$};
  % heights
  \draw[densely dotted] (0,6) -- (-2.4,6);
  \draw[<->] (-2.2,0) -- (-2.2,6) node[midway,left] {\small $30$ m};
  \draw[densely dotted] (8,2) -- (10.6,2);
  \draw[<->] (10.4,0) -- (10.4,2) node[midway,right] {\small $10$ m};
  % horizontal distance
  \draw[densely dotted] (0,6) -- (0,8.2);
  \draw[densely dotted] (8,2) -- (8,8.2);
  \draw[<->] (0,8.0) -- (8,8.0) node[midway,above] {\small $40$ m};
  % trajectory (1 unit = 5 m)
  \draw[very thick,blue!70!black,dashed,domain=0:40,smooth,variable=\x,samples=60]
      plot ({\x/5},{6+(\x-3*\x*\x/80)/5});
  % launch
  \draw[densely dotted] (0,6) -- (1.3,6);
  \draw[very thick,->,blue!70!black] (0,6) -- ++(45:1.2) node[above left=-1pt] {\small $u$};
  \draw (0.6,6) arc (0:45:0.6); \node at ($(0,6)+(22:0.95)$) {\small $45^\circ$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.05,>=Stealth]
  % faint terrain
  \fill[gray!10] (-1.6,0) -- (0,6) -- (3.464,0) -- cycle;
  \draw[gray!50] (-1.6,0) -- (0,6) -- (3.464,0);
  \fill[gray!10] (6,0) -- (8,2) -- (9.6,0) -- cycle;
  \draw[gray!50] (6,0) -- (8,2) -- (9.6,0);
  \draw[gray!60] (-2.0,0) -- (10.2,0);
  \fill[pattern=north east lines,pattern color=gray!60] (-2.0,-0.22) rectangle (10.2,0);
  % imaginary slope
  \draw[very thick,black!80] (0,6) -- (8,2);
  \node[rotate=-26.565,above] at (3.9,4.05) {\small imaginary slope, $L = 20\sqrt5$ m};
  \fill (0,6) circle (1.8pt) node[above left] {$A$};
  \fill (8,2) circle (1.8pt) node[below right] {$B$};
  % beta at B
  \draw[densely dotted] (8,2) -- (6.2,2);
  \draw (6.7,2) arc (180:153.435:1.3);
  \node at ($(8,2)+(166.7:1.75)$) {\small $\beta$};
  % trajectory
  \draw[thick,blue!70!black,dashed,domain=0:40,smooth,variable=\x,samples=60]
      plot ({\x/5},{6+(\x-3*\x*\x/80)/5});
  % tilted axes at A
  \draw[->,thick] (0,6) -- ++(-26.565:2.3) node[below left=-1pt] {$x'$};
  \draw[->,thick] (0,6) -- ++(63.435:2.0) node[above] {$y'$};
  % launch vector and alpha
  \draw[very thick,->,blue!70!black] (0,6) -- ++(45:1.7) node[above right=-2pt] {$u$};
  \draw[blue!70!black] ($(0,6)+(-26.565:0.75)$) arc (-26.565:45:0.75);
  \node[blue!70!black] at ($(0,6)+(9:1.0)$) {\small $\alpha$};
  % gravity decomposition on the ball
  \coordinate (P) at (4,7);
  \fill[blue!70!black] (P) circle (2pt);
  \draw[very thick,->,red!75!black] (P) -- ++(0,-1.3) node[right] {\small $g$};
  \draw[thick,->,orange!85!black] (P) -- ++(-26.565:0.581) coordinate (Ga);
  \draw[thick,->,orange!85!black] (P) -- ++(-116.565:1.163) coordinate (Gp);
  \draw[densely dotted,gray] (Ga) -- ($(P)+(0,-1.3)$) -- (Gp);
  \node[orange!85!black,anchor=west] at ($(Ga)+(0.05,0.12)$) {\small $g\sin\beta$};
  \node[orange!85!black,anchor=east] at ($(Gp)+(-0.05,0)$) {\small $g\cos\beta$};
  % dimensions
  \draw[<->] (0,-0.7) -- (8,-0.7) node[midway,below] {\small $40$ m};
  \draw[densely dotted] (8,2) -- (10.4,2);
  \draw[densely dotted] (8.9,6) -- (10.4,6);
  \draw[<->] (10.2,2) -- (10.2,6) node[midway,right] {\small $20$ m};
\end{tikzpicture}
\end{center}
